...
// HVL Info ESP32-S3 SuperMini pilot (ESP32-S3FH4R2). Wi-Fi is normally OFF: BLE advertises while
...
constexpr uint8_t BOOT_PIN = 0; // BOOT on tested ESP32-S3 SuperMini
const char *FIRMWARE_VERSION = "0.3.0-s3-pilot";
...
void applyBeacon(JsonObjectConst config) {
  ...
  if (!bleStarted) {
    ...
    // HVL Info is a presence beacon, not a connectable BLE peripheral.
...
void startPortal() {
  ...
  // AP+BLE coexistence must be verified with the full firmware on physical ESP32-S3 hardware.
...
void loop() {
  ...
  // ESP32-S3 ROM firmware downloader (GPIO0 LOW on reset), not this web setup portal.
